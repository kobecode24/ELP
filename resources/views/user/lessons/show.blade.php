...
@section('content')
    <div class="pl-24  text-[#C0C4FC]  pt-20 w-full  overflow-hidden flex flex-row flex-wrap items-end justify-start pt-2 px-1 pb-[7.8px] box-border gap-[4px] leading-[normal] tracking-[normal] text-left text-xl ">
        <div class="flex flex-col items-start justify-start py-0 pr-[6.3px] pl-0 bg-[#401B9C[">
            <a href="{{route('user.courses')}}"><b class=" leading-[17px] inline-block min-w-[83px]">Our Courses</b></a>
        </div>
        ...
        <div class="flex flex-col items-start justify-start py-0 pr-[8.5px] pl-0">
            <a href="{{route('user.courses.show' , $lesson->chapter->course->id)}}"><b>{{$lesson->chapter->course->title}}</b></a>
        </div>
        <img class="h-4 w-4 overflow-hidden shrink-0" loading="lazy" alt="" src="{{ asset('images/right_svg.svg') }}">
        <a href="{{route('user.lessons.show' , $lesson->id)}}"><b class=" leading-[17px] inline-block min-w-[83px]">{{ $lesson->title }}</b></a>
    </div>
    ...
    <div class="flex justify-center pb-6">
        <div class="navigation flex items-center">
            <a href="{{ route('user.items.prev', ['courseId' => $lesson->chapter->course_id, 'type' => 'lesson', 'currentItemId' => $lesson->id]) }}" class="w-5 h-5 border border-black rounded-full p-4 flex items-center justify-center mr-4">
                <i class="fa-solid fa-angle-left"></i>
            </a>
            <a href="{{ route('user.items.next', ['courseId' => $lesson->chapter->course_id, 'type' => 'lesson', 'currentItemId' => $lesson->id]) }}" class="w-5 h-5 border border-black rounded-full p-4 flex items-center justify-center">
